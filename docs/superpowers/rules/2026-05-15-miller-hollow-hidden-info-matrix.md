...
| Information | Public Before End | Player Private | Spectator Before End | Host Observer | Public After End |
| --- | --- | --- | --- | --- | --- |
| Own role | No | Yes, own role only | No | Yes | Yes |
| Other player roles | No | Werewolf teammates only for Werewolves | No | Yes | Yes |
| Seer results | No | Seer only | No | Yes | Reveal-safe timeline only |
| Witch potion availability | No | Witch only | No | Yes | No unless summarized |
| Werewolf target | No | Werewolf/Witch context as allowed | No | Yes | Reveal-safe timeline only |
| Werewolf proposed target | No | Living Werewolves during Werewolf night | No | Yes | No unless committed through events |
...
| Observer ticket | No | Host transport only | No | Short-lived transport only | No |
